...
CREATE POLICY "Public generation writes are blocked"
  ON public.generations FOR INSERT WITH CHECK (false);

CREATE POLICY "Public generation updates are blocked"
  ON public.generations FOR UPDATE USING (false);
...
CREATE POLICY "Public settings inserts are blocked"
  ON public.settings FOR INSERT WITH CHECK (false);

CREATE POLICY "Public settings updates are blocked"
  ON public.settings FOR UPDATE USING (false);
...
INSERT INTO public.generations (slug, product_name, product_url, brief, cta_text, audience, tone, language, template, status, generation_mode, cost_usd, scene_plan, script_text, video_url, view_count, duration_seconds, created_at) VALUES
